...
namespace lab1cg
{
    class Opening:Filters
    {
        ...
        {
            Bitmap InterImage = new Bitmap(sourceImage.Width, sourceImage.Height);
            int[,] mask = Filters.mask;
            int MW = mask.GetLength(0);
            int MH = mask.GetLength(1);
            for (int i = MH / 2; i < sourceImage.Height - MW / 2; i++)
            {
                worker.ReportProgress((int)((float)i / InterImage.Height * 100));
            ...
            for (int i = MH / 2; i < sourceImage.Height - MW / 2; i++)
            {
                ...
                for (int j = MW / 2; j < sourceImage.Width - MH / 2; j++)
                {
                    ...
                    for (int k = -MH / 2; k <= MH / 2; k++)
                        for (int l = -MW / 2; l <= MW / 2; l++)
                        {
                            Color sourceColor = InterImage.GetPixel(l + j, k + i);
                            float Intensity = 0.299f * sourceColor.R + 0.587f * sourceColor.G + 0.114f * sourceColor.B;
                            float MaxIntensity = 0.299f * max.R + 0.587f * max.G + 0.114f * max.B;
                            if ((mask[l + MW / 2, k + MH / 2] == 1) && (Intensity > MaxIntensity))
                                max = sourceColor;
                        }
                    resultImage.SetPixel(j, i, max);
                }
            }
